    ...
    getAllComponentMetadata: function () {
      ...
      if (registry.components) {
        for (var componentName in registry.components) {
          ...
          // auto-register event handlers in metadata
      ...
      logger.debug("MD: "+JSON.stringify(allMetadata));
      return allMetadata;
    },
    /**
    ...
    invokeComponentByName: function (componentName, requestBody, sdkMixin, callback) {
      ...
      try {
        if (entityStatus && entityStatus.event) {
          // by default release the turn, developer can keep turn if needed in event handlers
          sdk.releaseTurn();
          ...
          if (component[eventName]) {
            ...
            let returnValue = component[eventName](entityStatus.event, entityModel, sdk);
